...
public class TensorflowClassifier implements Classifier {
  ...
  private native String classifyImageBmp(Bitmap bitmap,int fea);
  ...
  @Override
  public List<Recognition> recognizeImage(final Bitmap bitmap,int fea) {
    // Log this method so that it can be analyzed with systrace.
    Trace.beginSection("Recognize");
    final ArrayList<Recognition> recognitions = new ArrayList<Recognition>();
    for (final String result : classifyImageBmp(bitmap,fea).split("\n")) {
      ...
      //if (!title.isEmpty()) {
      //  recognitions.add(new Recognition(id, title, confidence));
      //}
    }
    Trace.endSection();
    return recognitions;
